...
class PrioritizedEpisodeReplayBuffer(EpisodeReplayBuffer):
    ...
    def sample(
        ...
        **kwargs,
        ...
        if batch_length_T and batch_length_T > 1:
            assert (
                not isinstance(actual_n_step, tuple) and actual_n_step == 1
            ), "When sampling prioritized sequences n-step must be 1."
            return self._sample_prioritized_sequences(
                batch_size_B=batch_size_B,
                batch_length_T=batch_length_T,
                beta=beta,
                include_extra_model_outputs=include_extra_model_outputs,
                to_numpy=to_numpy,
                lookback=int(kwargs.get("lookback", 1)),
                min_batch_length_T=int(kwargs.get("min_batch_length_T", 0)),
                total_segment_sum=total_segment_sum,
    ...
    def _sample_prioritized_sequences(
        self,
        *,
        batch_size_B: int,
        batch_length_T: int,
        ...
        to_numpy: bool,
        lookback: int,
        min_batch_length_T: int,
        total_segment_sum: float,
        max_weight: float,
    ) -> List[SingleAgentEpisode]:
        """Sample prioritized episode slices for stateful RLModules.

        DogFightEnv SAC LSTM uses Ray 2.54's new API stack, where learner batches
        must preserve an actual time axis. The upstream prioritized buffer only
        returns 1-step episodes, which ConnectorV2 pads to ``batch_length_T`` and
        produces ``seq_lens=1``. This path mirrors EpisodeReplayBuffer sequence
        slicing while keeping one priority tree index per sampled sequence.
        """
        sampled_episodes = []
        sampled_env_step_idxs = set()
        sampled_episode_idxs = set()
        sampled_n_steps = []
        num_resamples = 0
        debug_enabled = os.environ.get("DOGFIGHT_PRIORITIZED_SEQ_DEBUG") == "1"
        debug_limit = int(os.environ.get("DOGFIGHT_PRIORITIZED_SEQ_DEBUG_LIMIT", "20"))
        debug_count = 0
        ...
        while B < batch_size_B:
            ...
            if episode_ts + batch_length_T > len(episode):
                actual_length = len(episode)
            else:
                actual_length = episode_ts + batch_length_T
            if actual_length <= episode_ts:
                num_resamples += 1
                continue

            sampled_episode = episode.slice(
                slice(episode_ts, actual_length),
                len_lookback_buffer=lookback,
            )
            sample_weight = weight / max_weight
            sampled_episode.extra_model_outputs["n_step"] = InfiniteLookbackBuffer(
                np.full((len(sampled_episode) + lookback,), 1),
                lookback=lookback,
            )
            sampled_episode.extra_model_outputs["weights"] = InfiniteLookbackBuffer(
                np.full((len(sampled_episode) + lookback,), sample_weight),
                lookback=lookback,
            )
            if not include_extra_model_outputs:
                for key in list(sampled_episode.extra_model_outputs.keys()):
                    if key not in {"n_step", "weights"}:
                        sampled_episode.extra_model_outputs.pop(key, None)
            ...
            sampled_episodes.append(sampled_episode)
            sampled_episode_idxs.add(episode_idx)
            sampled_n_steps.append(1)
            self._last_sampled_indices.append(idx)

            if debug_enabled and debug_count < debug_limit:
                print(
                    "[DogFightEnv][PrioritizedSeqReplay][SAMPLE] "
                    f"batch_length_T={batch_length_T} actual_len={len(sampled_episode)} "
                    f"lookback={lookback} sampled_index={idx} "
                    f"episode_ts={episode_ts} weight={float(sample_weight):.6f}"
                )
                debug_count += 1
        ...
        num_episodes_per_sample = len(sampled_episode_idxs)
        num_env_steps_per_sample = len(sampled_env_step_idxs)
        sampled_n_step = sum(sampled_n_steps) / max(len(sampled_n_steps), 1)
        agent_to_num_steps_sampled = {DEFAULT_AGENT_ID: num_env_steps_sampled}
        agent_to_num_episodes_per_sample = {DEFAULT_AGENT_ID: num_episodes_per_sample}
        agent_to_num_steps_per_sample = {DEFAULT_AGENT_ID: num_env_steps_per_sample}
        agent_to_sampled_n_step = {DEFAULT_AGENT_ID: sampled_n_step}
    ...
    def update_priorities(
        ...
        """
        if hasattr(priorities, "detach"):
            priorities = priorities.detach().cpu().numpy()
        priorities = np.asarray(priorities, dtype=np.float64)
        if priorities.ndim == 0:
            priorities = np.full((len(self._last_sampled_indices),), abs(priorities))
        elif priorities.ndim > 1:
            priorities = np.nanmax(
                np.abs(priorities),
                axis=tuple(range(1, priorities.ndim)),
            )
        else:
            priorities = np.abs(priorities)
        if len(priorities) != len(self._last_sampled_indices):
            if priorities.size % len(self._last_sampled_indices) == 0:
                priorities = np.nanmax(
                    priorities.reshape(len(self._last_sampled_indices), -1),
                    axis=1,
                )
            else:
                raise ValueError(
                    "Priority count does not match last sampled sequence count: "
                    f"priorities={priorities.shape}, "
                    f"indices={len(self._last_sampled_indices)}"
                )
        ...
        for idx, priority in zip(self._last_sampled_indices, priorities):
            ...
            priority = max(float(priority), 1e-12)
