...
class SACCatalog(Catalog):
    ...
    def _determine_components_hook(self):
        ...
        actor_lstm_layers = int(
            self._model_config_dict.get("dogfight_actor_lstm_num_layers", 1)
        )
        if actor_lstm_layers != 1 and isinstance(
            self._encoder_config, RecurrentEncoderConfig
        ):
            # DogFightEnv sequence_v1 keeps the actor path on RLlib's built-in
            # LSTM encoder but allows YAML to request stacked LSTM layers.
            self._encoder_config.num_layers = actor_lstm_layers
    ...
    def __init__(
        ...
        self.qf_head_hiddens = self._model_config_dict.get(
            "dogfight_q_head_fcnet_hiddens", self.pi_and_qf_head_hiddens
        )
        self.qf_head_activation = self._model_config_dict.get(
            "dogfight_q_head_fcnet_activation", self.pi_and_qf_head_activation
        )
        ...
        dogfight_lstm_scope = self._model_config_dict.get(
            "dogfight_lstm_scope", "actor_only"
        )
        qf_head_input_dims = (
            (
                int(
                    self._model_config_dict.get(
                        "dogfight_q_lstm_cell_size",
                        self._model_config_dict["lstm_cell_size"],
                    )
                ),
            )
            if dogfight_lstm_scope == "actor_critic"
            else self._dogfight_q_mlp_output_dims()
        )

        self.qf_head_config = MLPHeadConfig(
            input_dims=qf_head_input_dims,
    ...
    def _dogfight_q_fcnet_hiddens(self) -> list[int]:
        return list(
            self._model_config_dict.get(
                "dogfight_q_fcnet_hiddens",
                self._model_config_dict["fcnet_hiddens"],
            )
        )

    def _dogfight_q_fcnet_activation(self) -> str:
        return self._model_config_dict.get(
            "dogfight_q_fcnet_activation",
            self._model_config_dict["fcnet_activation"],
        )

    def _dogfight_q_mlp_output_dims(self) -> tuple[int, ...]:
        q_hiddens = self._dogfight_q_fcnet_hiddens()
        if q_hiddens:
            return (int(q_hiddens[-1]),)
        return self.latent_dims

    def _dogfight_build_mlp_encoder_config(
        self,
        input_space: gym.Space,
        hiddens: list[int],
        activation: str,
    ) -> MLPEncoderConfig | None:
        if not hiddens:
            return None
        return MLPEncoderConfig(
            input_dims=input_space.shape,
            hidden_layer_dims=hiddens[:-1],
    ...
    def build_qf_encoder(self, framework: str) -> Encoder:
        ...
        if self._model_config_dict.get("dogfight_lstm_scope") == "actor_critic":
            q_hiddens = self._dogfight_q_fcnet_hiddens()
            q_activation = self._dogfight_q_fcnet_activation()
            # DogFightEnv Ray 1.9.2 RNNSAC-compatible Q contract:
            # Q/twin/target Q all encode [obs, action] in this exact order before
            # the recurrent layer. The Q state itself is zero-initialized in the
            # torch module helper, not replayed from the actor STATE_IN.
            self.qf_encoder_config = RecurrentEncoderConfig(
                input_dims=input_space.shape,
                ...
                hidden_weights_initializer=self._model_config_dict[
                    "lstm_kernel_initializer"
                ],
                hidden_weights_initializer_config=self._model_config_dict[
                    "lstm_kernel_initializer_kwargs"
                ],
                hidden_bias_initializer=self._model_config_dict[
                    "lstm_bias_initializer"
                ],
                hidden_bias_initializer_config=self._model_config_dict[
                    "lstm_bias_initializer_kwargs"
                ],
                batch_major=True,
                num_layers=int(
                    self._model_config_dict.get("dogfight_q_lstm_num_layers", 1)
                ),
                tokenizer_config=self._dogfight_build_mlp_encoder_config(
                    input_space,
                    q_hiddens,
                    q_activation,
                ),
        ...
        q_hiddens = self._dogfight_q_fcnet_hiddens()
        self.qf_encoder_hiddens = q_hiddens[:-1]
        self.qf_encoder_activation = self._dogfight_q_fcnet_activation()
        ...
        self.qf_encoder_config = MLPEncoderConfig(
            ...
            output_layer_dim=self._dogfight_q_mlp_output_dims()[0],
