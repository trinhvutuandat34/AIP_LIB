        ...
        *,
        ...
        def mask_like(value):
            if loss_mask is None:
                return None
            mask = loss_mask.bool().to(device=value.device)
            while mask.ndim < value.ndim:
                mask = mask.unsqueeze(-1)
            return mask.expand_as(value)

        def masked_mean(value):
            mask = mask_like(value)
            if mask is None:
        ...
        # Learners - we will re-concatenate the produced TD-error tensors to yield
        # a 1:1 representation of the original batch.
        td_error_for_log = td_error
        td_error_mask = mask_like(td_error)
        if td_error_mask is not None:
            td_error_for_log = torch.where(
                td_error_mask,
                td_error,
                torch.zeros_like(td_error),
            )
        self.metrics.log_value(
            key=(module_id, TD_ERROR_KEY),
            value=td_error_for_log,
            reduce="item_series",
        )
        # Log other important loss stats (reduce=mean (default), but with window=1
        # in order to keep them history free).
        self.metrics.log_dict(
            {
                ...
                "log_alpha_value": torch.log(alpha)[0],
                "target_entropy": self.target_entropy[module_id],
                LOGPS_KEY: masked_mean(fwd_out["logp_resampled"]),
                QF_MEAN_KEY: masked_mean(fwd_out["q_curr"]),
                QF_MAX_KEY: masked_max(fwd_out["q_curr"]),
                QF_MIN_KEY: masked_min(fwd_out["q_curr"]),
                TD_ERROR_MEAN_KEY: masked_mean(td_error),
            },
            key=module_id,
            window=1,  # <- single items (should not be mean/ema-reduced over time).
        )
