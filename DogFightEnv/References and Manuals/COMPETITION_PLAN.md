...
| F27-**SCOPE** | **F27 blocks less than it looks like it does — separate "is `SERVER_IP` correct" from "does the reconnect logic survive a real connection loss" (2026-08-19).** The first genuinely needs the organizers and has no workaround. The second does not: `scripts/live_dq_rehearsal.py` takes any `--server-ip`/`--server-port`, and `supervise_client`/`ResilientActionProvider` have only ever been tested with `time.sleep` mocked in-process (`scripts/verify_resilience.py`) — never against a real OS socket. `Windows/BattleServer_V0.2/DogFightViewer.exe`, launched locally, gives the reconnect logic a real UDP endpoint to lose and regain a connection against. That is not a stand-in for the competition server and proves nothing about `SERVER_IP` — it is a legitimate, independently useful test of whether the client-side fault-handling code works under real network timing at all, which is answerable now regardless of when organizers publish an address. | **[D] partially actionable now**|
...
| F29 | **The champion controller envelope was never checked against a live tactical layer, and one lever COMPLETELY REVERSES VERDICT (2026-08-13).** Six configs re-swept via the peer rig (asymmetric, not symmetric — one side varies, target pinned at the reference champion 2500m/45°/throttle-off), N=30 each, `match_base`. Control (2500/45 vs itself): **8W/14D/8L**, damage 5.43/8.31, WEZ 690. | **[M]** |
| F29-**THROTTLE-REVERSED** | **Throttle authority, upgraded from "harmful" (F2-CHAMPION/F14) to the single best lever measured this session.** `--ownship-vptrack-throttle 1` vs the pinned champion: **14W/11D/5L**, damage **11.36/5.37**, WEZ **1525**. Every metric moves favourably together — wins nearly double (8→14), damage dealt more than doubles, damage taken nearly halves, WEZ more than doubles — which is the clean multi-metric pattern F5's "wins up, damage down" warning says to trust, not the confounded kind F26-GATE1-REJECTED found. Likely mechanism: closing speed was worthless against a tree that never converted a position; against one that now runs Gate 3's energy tactics and Gate 4's offensive selector for real, throttle authority to manage energy is genuinely load-bearing. **NOT YET ADOPTED** — one N=30 on one seed, however large the effect, is the bar this register has repeatedly said is not enough to ship on (E1: ±8.4% SE at N=30). Needs a second seed before changing `student/controller_providers.py`'s shipped throttle setting. | **[M] reversal, needs confirmation** |
| F29-**THROTTLE-CONFIRMED** | **ADOPTED 2026-08-13 (commit `e2634f4`) — confirmed across three independent seeds and now set explicitly in `my_submission.py`.** Seeds 0/1000/2000, N=30 each through the peer rig: **14W/11D/5L, 10W/15D/5L, 9W/16D/4L** against the control's 8W/14D/8L. **Losses fall in every seed** (8 → 5, 5, 4) and damage taken falls monotonically (8.31 → 5.37 → 4.64 → 3.21). Pooled **N=90: win 36.7% / loss 15.6%** vs control's 26.7%/26.7% — the loss rate is cut nearly in half. Win-rate gain is noisier (14/10/9) but never negative, so the defensive improvement is the robust part and the offensive upside is real but seed-dependent. Set **explicitly at the call site**, not via `DOGFIGHT_VPTRACK_THROTTLE`, whose default is off — relying on an env var means the submission silently reverts on any shell that does not export it, competition day included. Verified wired with the env var unset: `build_action_provider()` → `GLimitedProvider` → `VPTrackingProvider(throttle_control=True)`. **Methodological note:** measured through the peer rig from the start, which is why it survived where F26-GATE1-REJECTED did not. | **[M] ADOPTED** |
| F29-**RANGE-TIGHTER** | 2000m/30° (a new, previously-untested config, tighter than any prior sweep) also reads better than the champion: **11W/12D/7L**, damage 7.36/5.03, WEZ 1112. 2800m/45° is a smaller, less clear improvement: 9W/13D/8L, damage 5.95/5.67, WEZ 945. Direction of the effect (tighter range and adding throttle both help) is consistent with the tree now actually contesting the merge instead of orbiting past it. | **[M]** |
| F29-**CONFIRMS** | **LOS 60° and range 3000m both confirm their pre-F25 verdicts as inferior**, now measured live: LOS60 **6W/16D/8L** (was 3W/16D/11L pre-F25); range3000 **5W/11D/14L** (was 4W/17D/9L pre-F25). Both still lose. | **[M] confirms prior** |

| F30 | **A2 CLOSED — `GUNTRACK_RANGE_GAIN` is a measured NULL in both geometries, and it is ~70 % masked on the shipped path anyway (2026-08-13).** A2 had been "UNVALIDATED" since 2026-08-05 because the harness could not resolve it; F26-OWED deferred it again because it is a C++ constant, not an XML threshold. Now tested properly: two DLLs built from identical source differing **only** in `GUNTRACK_RANGE_GAIN` (0.0006f vs the documented exact-revert 0.0f), deployed under new names (`AIP_BASE_a2on.dll` / `AIP_BASE_a2off_target.dll`) so no protected runtime asset was touched — `AIP_BASE*.dll` verified unchanged at md5 `524adc75…` throughout. Run BT-vs-BT (**not** vptrack — see F30-MASKED), N=30 per geometry, ours gain-ON vs peer gain-OFF, each against a both-ON control:<br><br>`match_base`: **6W/23D/1L**, 2.72/1.00, WEZ 967 — versus the both-ON reference's **6W/23D/1L**, 2.72/1.01, WEZ 967. Indistinguishable.<br>`obfm_offensive`: **30W/0D/0L**, 17 kills, 27.10 dealt, WEZ 4149 — versus the both-ON control's **30W/0D/0L**, 18 kills, 27.31 dealt, WEZ 4121. Indistinguishable; **the 30/30 is the GEOMETRY, not the gain** (OBFM starts the ownship advantaged and the live tree converts it every time either way). That control was run specifically because the raw 30/0/0 could not otherwise be attributed. | **[M] null** |
| F30-**WHY** | **Mechanism, from a gate trace rather than inference.** At `match_base` BT-vs-BT, `Gun_Track` **wins zero ticks** in the traced window while twelve other nodes execute (`Gate4_OneCircleFight` 310, `Gate2_FlatScissors` 168, `Gate2_Beam_NoseToTail` 131, `Tail_LeadPursuit` 74, `Gate3_AnglesTactics` 56, …). A2's gain sits on a node that does not run there. WEZ contact averages only ~32 steps/episode at that geometry, so the node may fire briefly outside the traced window — but the A/B shows outcomes do not change either way. **Do not re-open A2 on the theory that it was never given a chance: it was, in the one geometry (OBFM) where the gun band is genuinely held, and it changed nothing.** | **[M]** |
| F30-**MASKED** | **A2 is largely moot for the SUBMISSION regardless of the above, and this is the decision-relevant part.** With `throttle_control=True` now shipped (F29-THROTTLE-CONFIRMED), `VPTrackingProvider._range_throttle` owns **70 %** of throttle inside the engagement envelope (`THROTTLE_AUTHORITY = 0.7`), blending only 30 % of the BT's value — and it drives to **`TARGET_RANGE_M = 220.0`**, the *same* setpoint as `GUNTRACK_TARGET_RANGE_M = 220.0f`. Its own docstring says the two "agree rather than fight". `Task_GunTrack` only runs at 152.4–914 m / own ATA < 8°, which is entirely **inside** the 2500 m / 45° envelope. So on `MODE="vptrack"` A2 is 70 % attenuated *and* directionally redundant. **This is why the A/B was run BT-vs-BT**: testing it on the shipped config would have produced a null meaning "masked", not "ineffective" — the misleading-null class this register has been burned by repeatedly. A2 is a `MODE="bt"` fallback question only. **Verdict: leave `GUNTRACK_RANGE_GAIN` at 0.0006f** (harmless, and the fallback is where it could ever act); spend no further effort here. | **[M] moot for submission** |

| F31 | **v9_residual completed — the FIRST campaign in this project's history to reach `status: completed` rather than being stopped or voided — but the trained policy actively harms performance at its own trained composition (2026-08-13 → 2026-08-17, ~25h).** Launched on `student/residual_training_wrapper.py` (`final = BT_action + 0.35*policy_action`, composed DURING training rather than only assembled at inference — v1-v8 all trained standalone). Wiring verified BEFORE and AFTER the run, not assumed: runtime chain built live is `GLimitWrapper → ResidualBTWrapper → MatchScenarioWrapper → ... → DogFightWrapper` (G limiter outermost, correct); the ownship-side BT ticks with **0 failures** over live steps; both `stage_14_match_base` and `stage_15_full_dogfight` bundles are weight-healthy (no NaN, unlike v4). | **[M]** |
| F31-**RESULT** | **The final bundle, evaluated at EXACTLY the composition it was trained under (`hybrid` mode, scale 0.35, `match_base`, N=30) is worse than doing nothing.** `0W/22D/0L`, damage 0.61/0.00, WEZ 46 -- against the plain-BT reference's `6W/23D/1L`, damage 2.72/1.01, WEZ 967, **zero self-crashes**. **8 of 30 episodes (27%) end in the OWNSHIP crashing itself** (`end_condition: "ownship altitude below min"`, ownship health 1.0 right up to the crash -- a flight-control failure, not combat damage). Every metric collapsed relative to the BT baseline the policy was meant to correct. | **[M] harmful, not adopted** |
| F31-**NOT-A-WIRING-BUG** | **Ruled out, not assumed, before concluding this is a training result rather than a repeat of the "shipped inert" pattern.** A stage-0 crash-rate parity between v9 and standalone v8 looked exactly like the wrapper's own designed degrade-to-passthrough path firing (`"BT unavailable -> policy action passed through unchanged"`); investigated directly rather than accepted. Pure BT alone (`residual_scale=0.0`) run through stage 0's exact config, N=15: **0 ownship crashes**, every episode surviving to the stage's own scripted termination. So the BT mechanism itself is safe standalone; the crashes are the trained residual's own contribution. Separately found and unrelated: stage 0's `target_mode="fixed"` target has no active flight control and falls out of the sky on its own at ~3406 steps in every episode, ending as a DRAW (`_classify_outcome` only counts `"ownship altitude below min"`/`"FDM Update Fail"` as crash) -- a pre-existing curriculum quirk present since v1, not specific to this campaign, and not the explanation for the ownship-crash finding above. | **[M]** |
| F31-**NEXT** | **Not adopted; no further compute spent chasing a rescue without direction.** Candidates for a future attempt, untested: a smaller `residual_scale` during training itself (0.35 may be too much authority for an early-training network to wield safely over a ~3400-step episode, even though it matches the shipped inference default); more stages/iterations specifically on `match_base`-adjacent geometry before `full_dogfight`; reward shaping that penalises the residual's own contribution to instability separately from outcome reward. None attempted -- this is a documented negative result, not a partially-tuned one. | **[D] deferred, needs a decision** |

| F32 | **Controller fine-tune search around the throttle-on champion found NOTHING that robustly beats 2500m/45° — the one promising result reversed on a second seed (2026-08-13).** With throttle-on now the real shipped baseline (F29-THROTTLE-CONFIRMED), re-ran the champion sweep with the target ALSO pinned at throttle-on (the honest reference — every earlier sweep pinned the target at the stale throttle-off default), then grid-searched range/LOS around the two points that had looked promising. Six configs, N=30, `match_base`: control (2500/45) **7W/13D/10L**; 2000/30 **6W/15D/9L**; **2200/35 10W/12D/8L** (net +2, every metric favourable — the standout); 1800/30 **7W/16D/7L**; 2000/45 **5W/15D/10L**; 2300/35 **8W/12D/10L** (net -2 to -5 elsewhere). | **[M]** |
| F32-**REVERSED** | **2200/35 was re-run on an independent seed (1000) before being trusted, per this session's own standing rule after F29. It reversed completely:** seed 0 **10W/12D/8L**, dealt 10.04/taken 5.62, WEZ 2035; seed 1000 **6W/11D/13L**, dealt 4.12/taken 8.42, WEZ 727. Pooled N=60: win 26.7% / **loss 35.0%** -- worse than the control's single-seed 23.3%/33.3%, not better. **This is the opposite outcome from F29-THROTTLE-CONFIRMED, where losses fell in all three independent seeds** -- the contrast is the point: `match_base`'s noise floor (E1, SE~8.4% at N=30) is large enough to produce a seemingly-decisive, every-metric-favourable single-seed result that is pure variance. **Verdict: the champion controller envelope (2500m/45°, throttle-on) is not improved on by anything found in this search and remains what ships.** No change to `my_submission.py`. Further single-seed range/LOS perturbation is not a good use of remaining runway -- the effect sizes being searched for are comparable to or smaller than the measurement noise. | **[M] reversed, not adopted** |

| F33 | **`CORNER_HOLD` re-tested post-F25, following the same logic that reversed throttle — and this one CONFIRMS its pre-F25 "harmful" verdict rather than reversing (2026-08-19).** `corner_speed_probe.py` originally found forcing 440 kt (inside the doctrine rate band, section G) harmful against a dead tree: 73.1% win rate but 0 kills, damage collapsed 96%. Since F29-THROTTLE-CONFIRMED reversed an equally-confident-sounding pre-F25 "harmful" verdict, this deserved the same re-check. Peer rig, N=30, `match_base`, ours `--ownship-vptrack-corner 1` vs the pinned champion (2500m/45°/throttle-on): **4W/13D/13L**, damage 4.56/**11.61**, WEZ 581 -- against the throttle-on control's 7W/13D/10L, damage 7.12/8.49, WEZ 1052. **Worse on every axis.** | **[M] confirms prior** |
| F33-**WHY** | **Mechanism, not just statistics.** `corner_hold` and the now-shipped range-based `throttle_control` are **mutually exclusive** in `student/controller_providers.py` (`if self.corner_hold: ... elif self.throttle_control: ...`), so enabling corner-hold does not ADD speed management on top of the proven throttle win -- it REPLACES a range-aware, already-confirmed-beneficial throttle law with a range-blind, fixed-440kt one. Given range-based throttle is independently confirmed across 3 seeds (F29), trading it away losing ground is the mechanically expected outcome, not a surprise. **Does not rule out a FUSED range+corner-speed law** -- that is unbuilt, would need real code (not a flag flip), and is not recommended without a specific reason to think it beats the simpler law already shipping. | **[M]** |

| F34 | **v10_residual completed (2026-08-18→19, ~22h) — the instability from F31 is GONE, but the policy contributes nothing, and the way it fails is different and worth reading carefully (2026-08-19).** One variable changed from v9: `residual_scale` 0.35 -> 0.10, testing F31's diagnosis directly. Bundle weight-healthy (0 NaN/Inf, 7 tensors, 71,945 params). Evaluated at its own trained composition (`hybrid`, scale 0.10, `match_base`, N=30 vs BT): **2W/26D/2L**, damage **0.03/1.19**, **WEZ 0/30**, kills 0, **0 self-crashes**. | **[M]** |
| F34-**COMPARE** | Against BT alone (the thing the residual is meant to improve on): 6W/23D/1L, 2.72/1.01, WEZ **967**. **v10's composed aircraft achieves zero WEZ contact in all 30 episodes** -- it never once holds range and angle together, where the unmodified BT it's supposed to correct does so 967 times. The 2 recorded "wins" are phased-scoring timeout artifacts (damage differential on essentially-zero absolute damage, 0.03 total across the whole run), not real engagements. Against v9 specifically: **the flight-control instability is gone** -- zero episodes end in `"ownship altitude below min"` (was 8/30, 27%) -- confirming F31's diagnosis that 0.35 was too much authority for an undertrained network. But the fix traded one failure mode for another: v9 destroyed itself with too much authority: v10 survives but **suppresses the BT's already-functional behavior instead of adding to it**, even at less than a third the scale. | **[M]** |
| F34-**READING** | **Two systematic attempts, two different failure modes, same bottom line: no version of naive residual training tested has produced a policy that helps.** v9 (0.35): active, destructive, catastrophic. v10 (0.10): passive, suppressive, inert. Between them they bracket a real design problem, not a tuning problem -- there is no evidence yet that a scale exists where this approach adds value rather than either overriding the BT harmfully or damping it uselessly. **Not recommended to try a third scale point blind** (e.g. 0.20) without a specific reason to think the relationship is smoothly interpolable between "destroys" and "does nothing" rather than simply having no useful middle. Candidates that change the APPROACH rather than the scale, none attempted: reward shaping that penalises the residual's own deviation from the BT baseline directly (encouraging small helpful nudges over doing-nothing-or-everything); a curriculum-staged residual_scale that starts near 0 and ramps up only once the policy demonstrates it can survive; or accepting the floor (`vptrack`, throttle-on, F25 + F29) as the shipped answer and redirecting remaining runway to the DQ workstream, which has had zero campaigns and is unambiguously higher-leverage against the clock. | **[D] decision owed** |

| F36 | **The SHIPPING config's win rate vs the rule-based BT had never actually been measured — filled 2026-08-19.** Every post-throttle measurement (F29, F32) was peer-vs-peer; the standing vs-BT figure (10W/18D/2L) predated `throttle_control=True`. Measured now, N=30, `match_base`, `vptrack --ownship-vptrack-throttle 1` vs `bt`: **14W/14D/2L = 46.7 %**, 7 kills, damage **10.29 / 1.09**, WEZ **1547**. Full mode comparison against the same rule-based opponent: `bt` alone 6W/23D/1L (20.0 %, 2 kills, 2.72/1.01, WEZ 967) → `vptrack` pre-throttle 10W/18D/2L (33.3 %, 4 kills, 7.71/1.04, WEZ 1191) → **`vptrack`+throttle 14W/14D/2L (46.7 %, 7 kills, 10.29/1.09, WEZ 1547)**. Clean monotonic improvement on every axis, and it **independently re-confirms F29's throttle adoption against a different opponent type** than the peer runs that originally justified it (33.3 → 46.7 % here). | **[M]** |
| F36-**HYBRID-VERDICT** | **Both hybrid modes are verified WORSE THAN DOING NOTHING against the rule-based BT.** `hybrid`@0.35 (v9 bundle): **0W/22D/0L, 0 kills, WEZ 46**. `hybrid`@0.10 (v10 bundle): **2W/26D/2L, 0 kills, WEZ 0** — literally zero WEZ contact where the unmodified BT achieves 967. Against `bt` alone's 20.0 % and the shipped `vptrack`+throttle's 46.7 %, **no hybrid configuration measured to date is viable**, and `MODE="vptrack"` (no RL bundle at all) remains correct. Note the reverted Gate 1 experiment's 21W/8D/1L (`gate1brk_matchbase.csv`) sits in `artifacts/eval/` and looks superior to everything here — **it is not a shippable config**; it was measured symmetrically and reversed to 6W/11D/13L when isolated through the peer rig (F26-GATE1-REJECTED). Do not anchor any estimate on it. | **[M]** |
| F36-**ESTIMATE** | **Do not quote 46.7 % as a competition expectation.** Our BT deals **1.09 total damage across 30 episodes** — it barely shoots back, exactly the F11-ZERO-DAMAGE caveat. The vs-BT number verifies the mode functions; it does not predict match outcomes. **The honest predictor is peer-vs-peer: ~30 % win / 15.6 % loss / ~53 % draw** (F29, pooled N=90), and **draws do not advance** under 제1안. Realistic read: roughly half of matches resolve at all, and of those we win about twice as often as we lose. Which end of the 30–47 % band applies depends entirely on whether the committee's baseline model can convert — if it resembles our own non-shooting BT, expect the higher figure; if it is a real competitor, expect the lower. | **[D]** |
| F37-**GATE1-DEFENSIVE-FIX** | **F26-GATE1-REJECTED's own named refinement, implemented and adopted (2026-08-20).** Added one `DECO_LOSCheck` (`Gate1_Jink_NotDefensive_OwnATA_Lt90`) inside the existing `Gate1_Jink` Sequence in both `Rule_forTraining.xml` and `Rule_real_eagle.xml`, immediately before `Task_JinkingTurn`: own ATA ≥ 90° fails the check, the Jink Sequence fails, and the Fallback drops through to the unmodified `Gate1_TheBreak` (`Task_Evade`) Sequence below. XML-only, no rebuild — unlike the rejected reorder, this adds one guard rather than moving anything, so Jink keeps first priority unchanged below ATA=90. Scored through the peer rig only (F26-RULE), both scenarios, **under the actual shipping config** (`throttle_control=True`): `obfm_defensive` **1W/0D/29L (3.3%) → 7W/4D/19L (23.3%)**, damage taken 0.917 → 0.550 (mean, −40%); `match_base` **14W/11D/5L (F29 control, seed 0) → 13W/11D/6L (43.3%)** — a 1-episode difference, within N=30 noise, i.e. **no measurable regression on the confirmed prelim geometry**. Throttle-off also checked (not what ships, but a second data point): `obfm_defensive` 1W/0D/29L → 6W/6D/18L (20.0%), taken 0.576; `match_base` two seeds (0, 1000) both land 7-8W/12-13D/10L against the F26 throttle-off control's 9W/14D/7L — a small, seed-consistent loss increase, smaller than the throttle-on/shipping-config result and not blocking since throttle-off isn't the submission path. Cutoff gate re-checked post-fix (`--ownship-vptrack-throttle 1 --ownship-vptrack-range-m 6000 --ownship-vptrack-los-deg 90` vs cutoff, N=30): unchanged, 30/30 wins, 1.129/0.000 damage — no interaction with the cutoff, which never reaches own-ATA≥90 geometry in the first place. **Adopted; live rule files carry the new guard.** | **[M] adopted** |
| F37-**WIDE-ENVELOPE-SCOPE** | **6000m/90° confirmed cutoff-only, never previously run through the peer rig.** Scored post-F37-GATE1-DEFENSIVE-FIX, both scenarios, N=30 seed 0: `match_base` **13W/8D/9L (43.3%), damage 0.428/0.478** — ties the 2500/45 champion on raw win rate (43.3% both) but takes **more than double the damage** (0.478 vs 0.230) and is the first config on this scenario to show a self-crash on either side (free=1, ours=1, both 0 at 2500/45). `obfm_defensive` **10W/3D/17L (33.3%), damage 0.360/0.577** — here it actually beats the 2500/45 config's 23.3%, recorded honestly rather than cherry-picked away, but damage taken is statistically the same (0.577 vs 0.550) so it isn't "safer," just higher-variance. **Verdict unchanged from the pre-measurement hypothesis**: on `match_base` — the confirmed prelim geometry — wide is a strictly worse trade (same wins, much more damage taken, new crash risk), so 6000/90 stays scoped to `--target-backend cutoff` runs; 2500/45/throttle-on remains the general-purpose and match-day default. | **[M]** |
| F37-**V11-CHARACTERIZED** | **The 2026-08-19 22:34 KeyboardInterrupt on v11_residual was a manual stop, not a training regression.** `curriculum_state.json` top-level `status: failed`, stage 3 interrupted at iteration 460/500 with no checkpoint. The stage-3 tail's flat, repeating `crash_rate=1.0`/`ep_wez_steps=0.0` rows initially looked like it might be reproducing v9's destructive pattern — but `v10_residual`'s own stage-3 log (identical hyperparameters through stage 5) shows the **exact same flat tail at the exact same point**, and v10 went on to complete all 15 stages without v9's self-crash problem. A fresh eval of v11's last-available checkpoint (stage 2 `final_bundle`, hybrid mode, scale 0.10, `match_base`, N=30 vs `bt`) confirms this: **1W/27D/2L, only 1/30 episodes ended before the clock at all** — no sign of v9's ~27% self-crash rate. Per the F34-READING decision table, this clears the bar for resuming rather than restarting: **Option A (resume from the stage-3 checkpoint boundary)** is the evidence-supported choice over launching v12 fresh. Not yet resumed — this is a multi-hour-to-multi-day commitment this close to prelims and is flagged for an explicit go/no-go rather than started silently. | **[D] resume recommended, not yet started** |

| F38-**ALTITUDE-RESOLVED** | **RESOLVED same day (2026-08-20, later): spawn is confirmed 15,000 ft (4572 m) by team member with firsthand knowledge of the Unreal level setup — exactly what was already coded.** Gate-traced 5 full 200 s `match_base` fights (vptrack+throttle vs peer BT, real opponent, ~51,780 ticks) at that altitude: `Task_ClimbToSafeAltitude` fires **0 times**. A real fight starting at 15,000 ft never comes anywhere near the 914 m trigger. The captured `logs/unreal_packets/` session that showed 0-1067 m altitude was evidently a different context (practice/test run, not representative of a real 15,000 ft-spawn match) — do not generalise from it. **F38-GATE0-REJECTED's revert stands** (it was harmful regardless of altitude), but the underlying worry driving F38-ALTITUDE — that we are tuned for a regime the match doesn't use — is closed. No further action needed here. | **[M] resolved** |
| F38-**ALTITUDE** (superseded, kept for history) | **The single biggest open risk found to date: we may be tuned for an altitude the match does not use, and it is worth roughly half our win rate (2026-08-20).** `student/match_scenario_wrapper.py` spawns at `MATCH_ALTITUDE_M = 4572 m`, and its own docstring flags this **PROVISIONAL** — "the deck pins only the SEPARATION… must be rechecked when the organizers publish exact figures". Real `MT_PlaneInfo` decoded from captured server sessions (`logs/unreal_packets/`, 13,708 frames with speed > 50 m/s) reports altitude **0–1067 m, median 604 m** — not 4572 m. Measured the same shipping config at both altitudes (`vptrack`+throttle vs `bt`, `match_base`, N=20, seed 0, `--min-altitude 300`), altitude the **only** variable: **4572 m → 35.0% win, WEZ 989, damage dealt 4.156, 3 kills, 1 self-crash**; **600 m → 15.0% win, WEZ 394, damage dealt 0.110, 0 kills, 4 self-crashes.** Damage dealt collapses **38×** and kills go to zero. Mechanism: `Task_ClimbToSafeAltitude` self-triggers at `SAFE_ALTITUDE_TRIGGER_M = 914 m`, and it is Gate 0 — first child of the outer Fallback — so when it fires it aims +5000 m up, firewalls throttle, returns SUCCESS, and **no tactical gate below it is evaluated**. 65% of real airborne frames sit below that trigger. This also matches the real-match report verbatim ("climbs away, never generates a firing solution, gets gunned"), and it is invisible to every local measurement in this register because they all spawn at 4572–7000 m. **`DOGFIGHT_MATCH_ALTITUDE_M` now overrides the spawn altitude so this regime is testable at all.** **The altitude is now a BLOCKING question for the organizers, alongside SERVER_IP (F27) — it decides whether our real win rate is ~35% or ~15%.** | **[M] blocking unknown** |
| F38-**GATE0-REJECTED** | **Restricting Gate 0 to a lower altitude band was tried and REVERTED — it is far worse, at BOTH altitudes, and the reason matters (2026-08-20).** Hypothesis from F38-ALTITUDE: if Gate 0 hijacks the match at 600 m, gate it to genuinely-critical altitude only. Implemented XML-only as an anonymous `<Sequence>` wrapping Gate 0 behind `DECO_AltitudeCheck UpDown="Less" Altitude="400"` (400 m chosen to sit ~95 m above the 305 m hard deck). Scored through the peer rig (peer = `cf23c8a`, ungated), `vptrack`+throttle both sides, `match_base`, N=30 seed 0: **600 m → 1W/0D/29L (3.3%), and 26 of 30 episodes end `ownship altitude below min`**; **4572 m → 7W/8D/15L (23.3%) with 13/30 self-crashes**, against an ungated peer-rig baseline of **13W/11D/6L (43.3%) with ZERO self-crashes**. **The prediction that it would be a no-op at 4572 m was WRONG**, and that is the lesson: aircraft *descend while manoeuvring*, so the gate also disabled `Task_ClimbToSafeAltitude`'s second trigger — the runaway-descent branch (`altitude ≤ 3000 m AND AltSpeed ≤ −150 m/s`) — which turns out to be load-bearing at every altitude. No vertical-speed decorator exists in the node library, so **XML cannot separate the two triggers**; only a C++ split (lower `SAFE_ALTITUDE_TRIGGER_M`, keep the rate trigger) could, and that needs a rebuild. **Reverted; live rule files verified byte-identical to `cf23c8a`.** Corrected reading of F38-ALTITUDE: Gate 0 firing often at 600 m is **not** simply a hijack — it is genuine survival work in a band only ~300 m above the hard deck, and the ungated config's 4-in-20 self-crashes are it earning its place. At a 600 m operating altitude there is no good threshold; the problem is that the whole tree, controller envelope and tuning were built and validated in a 4572–7000 m regime. | **[M] rejected** |

| F39-**ONE-SHOT-SUBMISSION** | **Confirmed by the team (2026-08-20): this is ONE locked submission, used for both the cutoff gate AND every prelim match — no resubmission window between them, no edits after Monday.** This changes the calculus on envelope choice: the mandatory >50% cutoff bar (organizers' own words: even accounting for the cutoff's own ~2-3-in-10 self-crash rate favouring us, ≤50% means reconsidering prelim participation entirely) is a hard, unappealable, one-shot gate, and gets weighted accordingly against marginal prelim tuning. | **[D]** |
| F39-**ENVELOPE-MARGIN** | **Shipped default changed 2500m/45° → 4000m/60° (`student/my_submission.py`), after checking the full space rather than picking one point.** Cutoff N=100: standard 76% win/73% earned (CI ~[67,83]) vs 4000/60 **100% win/95% earned (CI ~[96,100])** — the mandatory bar goes from "comfortably clear" to "as safe as measurement can show." Two other candidates checked and rejected: 4000/45 is strictly *worse* against a real peer (36.7% win, damage taken 0.293 vs standard's 0.230) despite its own 100% cutoff score; 2500/90 is worse everywhere (20.0% win). Confirmed 4000/60 does not cost prelim performance, via the peer rig (never a symmetric measurement): `match_base` win rate matches or slightly exceeds standard on two independent seeds (46.7%/46.7% vs 43.3%), and `obfm_defensive` win rate is identical (23.3% both) with damage taken barely moving (0.577 vs 0.550). The real, bounded cost is confined to `match_base`: damage taken roughly doubles there (0.48-0.53 vs 0.23) because the wider envelope holds the terminal law engaged longer — it changes the margin of wins, not whether we win. Net: materially larger cutoff safety margin at no measured win-rate cost in either real-opponent scenario tested. | **[M] adopted** |

| F40-**SILENCE-BLIND** | **The client cannot detect a silent server, and that is the exact failure mode a wrong `SERVER_IP` produces (found live 2026-08-21, fixed).** First real attempt at F27-SCOPE's rehearsal against an actual `DogFightViewer.exe`: the client connected and then received **0 frames for 45 s, with 0 reconnect attempts and no warning of any kind**. Mechanism, in the no-edit client (`src/dogfight/unreal/client.py:242-243`): `except socket.timeout: continue`. Server silence produces *only* recv timeouts, never an `OSError`, so `_receive_loop()` spins forever, `run()` never returns, and `supervise_client`'s reconnect — which triggers only on a return or a throw — is never armed. **Confirmed not to be our code:** the organizers' own `unreal_bt_client.exe` reported `rx_packets=0` against the same server in the same minute; when the reference client also gets nothing, the fault is upstream. **Fix (student space only, no boundary crossing):** `ResilientActionProvider` now timestamps every server-driven call (`call_count` / `last_call_monotonic` — `compute_action` is called once per PlaneInfo pair, so it is a direct proxy for "when did the server last speak"), and a new `_silence_watchdog()` in `student/submission_resilience.py` warns on silence. Wired at both live entry points and in `live_dq_rehearsal.py`. **Deliberately warn-only by default (`silence_reconnect_sec=0.0`)**, matching the organizers' reference client whose `--server-timeout-sec` is documented "Warn (log only)" — that is evidence silence is normal often enough that the reference does not reconnect on it, and reconnecting through a legitimate inter-episode pause would manufacture the very "network instability" RULES Sec 8 disqualifies for. Opt-in reconnect is implemented and verified working, and is gated on frames having *already flowed* (a never-received silence can only be a wrong address or an unstarted match, neither of which a reconnect fixes). **Enabling it needs a measured inter-episode pause from the organizers — asked in `ORGANIZER_QUESTIONS_20260821.md` Q2.** | **[M] fixed, warn-only shipped** |
| F40-**REHEARSAL-VACUOUS** | **The 2026-08-20 18:34 DQ rehearsal passed without testing anything — `--induce-faults` is a no-op on loopback.** It reports surviving 3 fault cycles, but it ran against the loopback stand-in and **Windows Firewall does not filter 127.0.0.1 traffic**, so the `netsh advfirewall` rules blocked nothing. The tell is inside its own artifact: 3 blocks × 6 s against a 0.2 s recv timeout yielded `fallback_calls=0, disconnects=0` — arithmetically impossible had the wire actually dropped. `--induce-faults` is meaningful **only against a server on another host**; the script now says so at the top. Replaced with a real local fault path: `scripts/_dq_loopback_server.py --silence-every-sec/--silence-for-sec` simply stops transmitting, which needs no firewall and no elevation. Two crashes fixed in that harness while proving it: `ConnectionResetError` (WinError 10054 — raised on a UDP *recv* after an earlier send drew ICMP port-unreachable) killed the server outright mid-rehearsal, and merely swallowing it **deadlocked** the server pinned to the client's dead pre-reconnect port; it must also reset `client_addr = None`. **Verified end-to-end:** warn path fires on both branches; opt-in reconnect genuinely recovers (frames resumed 654 → 1249 → 1776 across 3 forced reconnects, `fallback_calls=0`). `verify_resilience.py` and `verify_report_fixes.py` both still pass. | **[M] fixed** |
| F45-**ALL-MODES-VS-REAL-CUTOFF-N50** | **Every backend re-scored at N=50 against the organizers' actual `unreal_bt_client.exe` binary (not a reimplementation — `cutoff_provider.py` spawns it via `subprocess.Popen`, its decision code runs untouched at competition default `--cutoff-action-repeat 6`), 2026-08-21.** All 7 CSVs verified complete (51 rows each) before reading. | **[M]** |
| F45-**RESULTS** | `env_r6000_l90`: **50W/0D/0L, rule 100.0%, earned 100.0%, 0 self-crashes either side.** `env_r4000_l60` (**the SHIPPED config**): **50W/0D/0L, rule 100.0%, earned 96.0%** (2 of 50 wins were the cutoff's own self-crash, 0 ours). Then a cliff: `hybridgated_v10` 14.0% (7W/15D/28L), `bt` alone 6.0%/4.0% (3W/46D/1L), `rl_v12` 4.0%/**0.0%** (2W/24D/24L), `hybridvp_v10` 2.0% (1W/17D/32L), `hybrid_v10` **0.0%** (0W/15D/35L). | **[M]** |
| F45-**READING** | **Do not read the 100%/100% vs 100%/96% gap as a reason to reconsider 4000/60.** That comparison was already settled on a more decisive axis: `env_r6000_l90` was tested on `match_base` (F37-WIDE-ENVELOPE-SCOPE) and found to cost real-match performance — same win rate, materially more damage taken, first self-crash risk appearing — which is why 4000/60 shipped despite a marginally lower cutoff score. Both configs clear the organizers' >50% bar by roughly 50 points; the decision between them was never going to be made on the cutoff number. **The value of this run is the N=50 confirmation itself** (tighter CI than any prior cutoff measurement of the shipped config) and the RL/hybrid column. | **[M]** |
| F45-**RL-HYBRID-CONFIRMED-AGAIN** | **Every RL/hybrid mode is catastrophically below the bar against a real, independent opponent — not just our own BT (F36), now the organizers' binary too.** `end_condition` breakdown shows the identical self-crash shape F31/F34 found in training: `hybrid_v10` 35/50 and `hybridvp_v10` 32/50 end `ownship altitude below min`; `rl_v12` 24/50 (plus the 24 draws, it never once converts — 0.0% earned, matching F43's training-time result exactly, now on a different opponent). `hybridgated_v10`'s 14.0% is entirely explained by the D3-REBASED pattern recurring at N=50: **5 genuine kills against 28 self-crashes** — it is the only hybrid variant that ever earns a real win, at a self-destruction cost that swamps the gain. `bt` alone (6.0%/4.0%, 48/50 timeouts) confirms it is safe but nearly inert against a real opponent, consistent with why it is the fallback and not the submission. **No new information changes what ships** — `MODE="vptrack"`, `BUNDLE_DIR=None`, 4000/60/throttle-on — but this is now confirmed against an opponent that is not our own tuning target, closing the gap F36-ESTIMATE flagged ("depends on whether the committee baseline can convert"). | **[M] confirms shipped config, no change** |
| F44-**VPTRACK-CONFIG-DRIFT** | **`run_unreal_inference.py --mode vptrack` was silently flying the PRE-F29/F39 config, and the shipped values now have a single source of truth (2026-08-21).** Found while verifying flags for a command reference. `_build_action_provider_raw` built a bare `VPTrackingProvider(dll_name=args.bt_dll)`, so it took the module defaults — **2500 m / 45° / throttle OFF** — because the shipped `4000 m / 60° + throttle` existed only as literals inside `student/my_submission.py`. Same mode name, same flag, an order-of-magnitude different aircraft: that config measures **13.3 %** against the cutoff where the shipped one scores **100 %** (N=100, F39-ENVELOPE-MARGIN). Anyone reaching for `run_unreal_inference.py` for a live match or a diagnostic run — the natural thing to do, since it is the flagged CLI entry point — would have got the weak aircraft with no indication. **Fix:** `SHIP_ENGAGE_RANGE_M` / `SHIP_ENGAGE_LOS_DEG` / `SHIP_THROTTLE_CONTROL` now live in `student/controller_providers.py`, and **both** live entry points read them; `my_submission.py` references the constants instead of literals (values unchanged), and `run_unreal_inference.py` gains `--vptrack-range-m` / `--vptrack-los-deg` / `--vptrack-throttle` defaulting to them. **The class defaults (2500/45/off) are deliberately left alone** — every historical measurement in this register was taken at them, so changing those would silently re-interpret old results. **Verified** by constructing the provider from each entry point and comparing field by field: `4000.0 / 60.0 / True` on both, and the override path still yields `2500.0 / 45.0 / False`. Both guards still pass. | **[M] fixed** |
| F43-**V12-CLOSES-THE-RL-LINE** | **v12_standalone completed all 16 stages (7,298 iters, 2026-08-21 16:50) and returns the clean negative it was designed to return: the bottleneck was never residual composition — it is the reward/curriculum. The RL line is closed on evidence, not fatigue.** v12 was launched precisely as the missing control (F5e360e): v1-v8 were standalone but faced the DEAD tree (F25 landed after v8), v9/v10/v11 faced the live tree but were all residual, so **no campaign had ever trained a plain standalone policy against the live F25-fixed opponent** — meaning every residual failure had been read without a control for "can this reward/curriculum produce a working policy against this opponent at all". **It cannot.** **Training:** 0 of 16 stages met its own advance gate — all 16 exited on `max_iterations_reached`. Zero wins in any stage at any point across the whole campaign. Peak WEZ contact anywhere was **29 steps** in one two-circle stage; the unmodified BT alone scores **967** on `match_base`. Final stage ends `crash_rate=1.0, win_rate=0.0, ep_wez_steps=0.0, ep_reward_damage=0.0` across its last five real measurements. **Eval (N=30, `match_base`, `--ownship-backend rl` standalone as v12's own config instructed, vs `bt`; bundle passes the NaN/Inf health gate at 7 tensors / 71,945 params):** **0W / 3L / 7 timeout / 20 self-crash**, `end_condition` = `ownship altitude below min` ×20, **0 WEZ-contact episodes, 0 total WEZ steps, damage 0.0000 dealt / 7.7001 taken**. Verified complete: 31 CSV rows, closing `[eval] summary` block present (the F28 partial-CSV trap). **Consequence, stated plainly: v9's scale 0.35, v10's 0.10 and v11's `damage_scale`=60 were all tuning a dimension that was never the constraint.** Twelve campaigns, zero usable bundles. **A v13 on any variation of this reward/curriculum has no evidential basis and should not be launched.** `MODE="vptrack"` with `BUNDLE_DIR=None` is confirmed correct, now against a control rather than by elimination. **Incidental:** the observation-mismatch guard (`student/inference_providers.py::verify_bundle_observation`) refused a first eval attempt that left the runtime at `tactical16` while the bundle was trained on `real_eagle15`/`my_observation_v2` — it failed loudly instead of silently feeding the policy wrong features. That guard is doing real work; keep it. | **[M] closed** |
| F42-**SETUP-NOT-GUARDED** | **The reconnect supervisor never protected the setup that precedes it — a transient DLL lock forfeited the whole run, now fixed and verified on the packaged artifact (2026-08-21).** `supervise_client` guards everything from the moment the client object exists, but it is only reached after `activate_rule_xml`, `build_action_provider` (which loads the BT DLL) and `load_observation_hook` — **all three ran bare in `main()`**. Measured, not reasoned: with the BT DLL momentarily unavailable, `build_action_provider()` raises `FileNotFoundError` and `main()` exits immediately. The process is gone, it never connects, and RULES Sec 8 makes failing to connect a DQ path — the exact outcome the resilience layer exists to prevent, one step earlier in the sequence where nothing was watching. **Why it is a real match-day risk:** the submission is extracted from a zip minutes before it runs on a machine we do not control; Windows Defender scans a freshly written `.dll` on first access and can briefly hold it, an extract may still be flushing, a scanned volume can stall the first open. All transient, all currently fatal. **Fix:** `run_with_setup_retry()` in `student/submission_resilience.py` wraps the full build-and-run sequence (the retry must *enclose* `supervise_client`, since the rule-XML context manager has to stay open for the run's lifetime). Retries indefinitely and deliberately — a permanent fault is then loudly and repeatedly reported while the operator can still fix it in place and the run self-recovers, whereas exiting converts a fixable problem into a silent forfeit. `KeyboardInterrupt` always ends the loop; in `run_unreal_inference.py` **argparse stays outside the retry** so a bad CLI flag fails fast rather than spinning (verified: `--mode NOPE` still exits 2). **Verified on the PACKAGED artifact, not the source tree:** built the zip, extracted clean, hid `AIP_BASE.dll` inside the extract, launched → 4 `SETUP FAILED` retries logged with the real cause and the process stayed alive; restored the DLL → connected on attempt 1 and ran normally (1552 PlaneInfo → 776 CMD, exact 1:1 per frame pair). | **[M] fixed** |
| F42-**PACKAGE-SMOKE-TEST** | **The submission artifact now has an end-to-end test, not just a compliance check (2026-08-21).** `scripts/package_release.py` stages and *asserts* (protected assets present, frame fix imported) but never runs anything. Closed that: build the zip → extract to a clean directory → run `student/my_submission.py` **from the extract** against `scripts/_dq_loopback_server.py` replaying real captured `MT_PlaneInfo` frames, pointed at localhost via `DOGFIGHT_SERVER_IP`/`DOGFIGHT_SERVER_PORT`. **Result: 12.9 MB staged / 3.0 MB zipped (limit 1024 MB), 210 entries; RX 2490 PlaneInfo → TX 1245 MT_CMD — exactly one command per own/enemy pair, confirming `ACTION_REPEAT=1` on the wire; `own_plane.valid=True`, plane id assigned; zero resilient-fallbacks, zero reconnects, zero silence warnings.** This exercises the **live path** (`context.sim is None`) — the branch no local eval had ever touched and where both frame bugs lived. Also confirmed: the entry point is **cwd-independent** (launched from `C:\` with an absolute script path it still resolves its own assets and runs 1672→836), and stdout redirected to a file prints Korean correctly, i.e. the 2026-08-06 UTF-8 fix still holds under the unattended-launch shape that originally broke it. **Still unknown and worth asking:** whether the organizers expect a particular zip layout — ours puts files at the archive root rather than nested in a `real_eagle/` folder. | **[M]** |
| F41-**NODE-AUDIT** | **Standing check for unreachable nodes, built and run: the tree is HEALTHY — 57 of 60 named nodes succeed at least once, and the 3 that never fire are explained without a bug (2026-08-21).** Motivation: this project has shipped an unreachable tactical layer twice (F25 — all 17 named composites built with zero children, only 2 leaf nodes had ever run; F37 — `Task_Evade` structurally starved), and the organizers' cutoff model has the identical class of defect independently (`DECO_EnemyWEZCheck`, `DefenceTurn`, `JinkingTurn`, `JinkingTurnSelector`, `DECO_JinkingCoolTimeCheck` all compiled into `unreal_bt_client.exe`, referenced **zero** times by its active tree — verified by counting occurrences in the binary vs the recovered tree). Two independent teams on the same base tree both shipped a disconnected defensive layer, so this is a property of the base, not bad luck. New tool `scripts/audit_unreachable_nodes.py` parses the `AIP_BT_GATE_TRACE` `.nodes.txt` inventory plus any number of trace CSVs and reports two verdicts: NEVER-EVALUATED (the F25 shape) and NEVER-SUCCEEDS (evaluated, always FAILURE — the F37 shape, the one an outcome metric can never show you). Run across 4 geometries (`match_base`, `obfm_defensive`, `obfm_offensive`, `two_circle_headon`), `bt` both sides, 2 episodes each, full 200 s: only `Gate2_VerticalScissors`, `Gate2_RollingScissors` and `Gate4_LowYoYo` never succeed — 0 successes in **five** traces spanning both controller configurations. **Method note worth reusing:** a single geometry is not enough and will produce false positives — a defensive node legitimately stays silent in an offensive fight (`match_base` alone flagged 10 nodes; the cross-geometry union flagged 3). | **[M]** |
| F41-**SCISSORS-NOT-A-BUG** | **The obvious fix for F41-NODE-AUDIT was designed, pre-flighted, and ABANDONED before running — it would have measured a no-op, and the finding it was built on was misread (2026-08-21).** All three dead nodes are vertical-plane maneuvers, and the mechanism looked exactly like F37 repeating: `Task_FlatScissors` returns SUCCESS on every tick it is evaluated (300 S / **0 F** on `match_base`) and sits first in the scissors `Fallback`, shadowing `Task_VerticalScissors` / `Task_RollingScissors` beneath it. Proposed fix followed the F37 pattern (add ONE guard rather than reorder — F26-GATE1-REJECTED showed reordering reverses 2:1 under isolation): wrap `Task_FlatScissors` behind `DECO_EnergyRatioCheck UpDown="Less" Ratio="1.2"`, so it stops claiming when we hold an energy advantage. **Killed at pre-flight by measuring how often the guard would actually bite**, using the `energy_ratio` column already present in the gate traces: inside the scissors branch, `ratio > 1.2` occurs on **0 of 972 ticks on `match_base`**, 0 of 439 on `two_circle_headon`, 0 of 19 on the vptrack trace; median ratio inside the branch is **1.000**. The A/B would have burned ~4 h of contended CPU to return a null that looked like evidence. **Mechanism, and the correction it forces:** the branch only opens on mutual ATA > 45° inside 2000 m — a stalemate where neither aircraft can point — and energy has necessarily equalised by then, which is *what makes it a flat scissors*. The shadowed nodes' doctrinal precondition (energy advantage / vertical separation) does not co-occur with the branch's own entry condition, so `Task_FlatScissors` holding the claim there is **correct behaviour, not the F37 bug**. F37 was a genuine defect because `Task_JinkingTurn` monopolised a *defensive* tick where `Task_Evade` was doctrinally preferable; no equivalent case is demonstrated here. **The earlier "F37 shape repeating" read was pattern-matching on structure (always-SUCCESS node first in a Fallback) without checking whether the shadowed node was actually the better choice in that condition.** Live tree never modified (`git` clean, md5 `F49BACF8…` throughout). Variant preserved unapplied in `experiments/rule_variants/scissors_energy_gate_EXPERIMENTAL.md`. **`Gate4_LowYoYo` is not fixable in XML at all** — Gate 4's children self-gate entry conditions inside C++, so the only XML lever is a reorder, the shape already rejected once. **Standing conclusion: the audit found no actionable defect; do not touch the tree on its strength.** | **[M] negative, not adopted** |
| F40-**VIEWER-D3D11** | **F27-SCOPE's "launch `DogFightViewer.exe` locally" workaround does not work on this machine — the live rehearsal is blocked, not merely undone.** `D:\BattleServer_V0.3_Full\DogFightViewer.exe` (note: a **V0.3** build; F27-SCOPE says V0.2) loads the engine to ~216 MB then dies with `LowLevelFatalError [D3D11Util.cpp:213] CreateSwapChainResult failed with error DXGI_ERROR_NOT_CURRENTLY_AVAILABLE`. It needs a GPU-backed interactive desktop; a process launched from any non-interactive/automation context has no desktop and fails at swap-chain creation. **The same crash exists from 2026-08-20 12:26, i.e. it predates and is independent of this session.** Crash dumps: `%LOCALAPPDATA%\DogFightViewer\Saved\Crashes\`. **Trap worth remembering: do not infer "the server is running" from process presence.** Three instances sat up for 6 h at **7 MB working set with no main window** — never initialised — while still holding UDP 9999 and silently blackholing every packet from both our client and the organizers'. A live instance is hundreds of MB; check `WorkingSet64` and `MainWindowTitle`. **This does not block competing** (the organizers host the server; our submission is only a UDP client) — it blocks local live verification, which is why Q4 of `ORGANIZER_QUESTIONS_20260821.md` asks for a practice window. Only a launch by a human from the unlocked physical console has any chance of working here. | **[M] blocked** |
